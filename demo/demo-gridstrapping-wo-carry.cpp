...
void runSingleCiphertextBootstrappingExperiment(usint m, usint r, usint cyclo, usint bits){
	...
	for (usint j = 0; j < gridSize; j++) {
		...
		/*
		 * Even if we decrypt it we won't find won't able to see bootcipher because it
		 * is multiplied with nInverse
		 */
	}
	...
	for (usint i = 0; i < dim; i++) {
		...
		//Grid Addition algorithm starts
		for (usint j = 0; j < gridSize; j++) {
			//aiEncoding[j][0] += neg;
			minus_aiEncoding[j][0] += neg;
			minus_aiEncoding[j].SwitchFormat();
			auto first = RGSWOps<Element>::ClearRingMultiply(minus_aiEncoding[j], bootKey[i]); //first is gsw
			RGSWOps<Element>::AddOneInPlace(first); // first -> Q_i
			bootCipher[j] = RGSWOps<Element>::Multiply( bootCipher[j], first); // bootCipher[j] -> ACC_i; ACC_i = ACC_i x Q_i
			//extract sign
			auto signCipher = RGSWOps<Element>::ExtractMaskedCipherAltAlt(bootCipher[j]); //signCipher is gsw
			/*After RCE procedure, if the addition is < N, then signCipher = -1
			 *if the addition is > N, then signCipher = 1
			 *This implies that signCipher should be negated first and then multiplied
			 */
			signCipher->NegateSelf();
			{
			...
			bootCipher[j] = RGSWOps<Element>::Multiply(signCipher, bootCipher[j] ); //abs value operation
			/*if (j != gridSize - 1) {
		...
		for (usint j = 0; j < gridSize; j++) {
			auto nInvCipher = RGSWOps<Element>::ScalarMultiply(N, bootCipher[j]);
			auto ptxt = RGSWOps<Element>::Decrypt(nInvCipher, kpRGSW.secretKey);
			cout << "bootcipher at " << j << "-th index \n" << ptxt << "\n";
		}
