...
void runSingleCiphertextBootstrappingExperiment(usint m, usint r, usint cyclo, usint bits){
	...
	//create keygen for switching to GSW key : TO-DO
	...
	std::cout << "starting power cache Generation "<< '\n';
	start = currentDateTime();
	RGSWOps<Element>::GeneratePowerCache(cryptoParamRGSW, ell);
	end = currentDateTime();
	std::cout << "power cache Genetation took "<< (end - start) <<" ms to finish\n\n";
	...
	for (usint i = 0; i < dim; i++) {
		...
		acc = RGSWOps<Element>::Multiply(acc, first);
